...
router.get('/', function (req, res, next) {
	...
	/* Server online check */
	if (global.kea_server.server_active == 1) {

		/* Reload Stats */
		app.reload();

		/* Lists to store server network data */
		global.kea_server.subnet_list = [], subnet_util = [], subnet_assgn_addr_list = [], subnet_total_addr_list = [], subnet_free_addr_list = [],
		...
		shared_nw_count = global.kea_server.server_config[global.kea_server.svr_tag]['shared-networks'].length;
		...
		subnet_count = global.kea_server.subnet_list.length;
		for (var i = 1; i <= subnet_count; i++) {
			...
			/* Subnet pools parser */
			pools = global.kea_server.subnet_list[i - 1]['pools'];
			pool_range = '';
			if (pools) {
				pools.forEach(p => {
		...
		counters = template_render.set_template_variable(counters, "global.kea_server.run_status", run_status_html);
